...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'hardware' | 'tracker' | 'notepad' | 'history'>('dashboard');
  ...
  const [trackerSubTab, setTrackerSubTab] = useState<'meals' | 'bazar' | 'deposits' | 'wallet'>('meals');

  const [isConfigModalOpen, setIsConfigModalOpen] = useState<boolean>(false);
  const [billConfig, setBillConfig] = useState<MonthlyBillConfig | null>(null);

  // Dynamic Month States
  const [availableMonths, setAvailableMonths] = useState<string[]>(['July-2026', 'June-2026']);
  const [isNewMonthModalOpen, setIsNewMonthModalOpen] = useState<boolean>(false);
  const [newMonthPrevMonthId, setNewMonthPrevMonthId] = useState<string>('July-2026');
  ...
  const [noteCategory, setNoteCategory] = useState<'general' | 'purchase' | 'reminder' | 'todo'>('general');
  ...
  const [editingNoteCategory, setEditingNoteCategory] = useState<'general' | 'purchase' | 'reminder' | 'todo'>('general');
  const [editingNoteAmount, setEditingNoteAmount] = useState<string>('');
  const [editingNoteReminderDate, setEditingNoteReminderDate] = useState<string>('');
  ...
  const [activeCommentCell, setActiveCommentCell] = useState<{ type: 'bazar' | 'deposit'; day: number; userId: string } | null>(null);
  ...
  const handleInviteRoommate = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/home/invite', { nickname: inviteNickname });
      ...
      showAlert('Error', err.response?.data?.error || 'Failed to invite roommate.');
    } finally {
      setInviteLoading(false);
    }
  };

  const handleTogglePermission = async (memberId: string, currentHasControl: boolean) => {
    try {
      ...
      showAlert('Error', err.response?.data?.error || 'Failed to toggle permission');
    }
  };
  ...
  const handleCreateMonth = async () => {
    try {
      ...
      showAlert('Month Setup Failed', err.response?.data?.error || 'Failed to create month');
    }
  };
  ...
  const handleCreateNote = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/notepad', {
        ...
        reminderDate: noteCategory === 'reminder' && noteReminderDate ? noteReminderDate : null,
        userId: activeUserId,
        userName: activeUserName
      });
      setNoteText('');
  ...
  const handleSaveNoteEdit = async (noteId: string) => {
    try {
      await api.put(`/notepad/${noteId}`, {
        ...
        reminderDate: editingNoteCategory === 'reminder' && editingNoteReminderDate ? editingNoteReminderDate : null,
        userId: activeUserId,
        userName: activeUserName
      });
  ...
  const handleToggleTodoCompleted = async (noteId: string, currentCompleted: boolean) => {
    try {
      await api.put(`/notepad/${noteId}`, {
      ...
      fetchNotes();
      showAlert('Success', `Todo marked as ${!currentCompleted ? 'completed' : 'incomplete'}!`);
    } catch (err: any) {
      console.error('Error toggling todo completion:', err);
      showAlert('Error', err.response?.data?.error || 'Failed to update todo status');
    }
  };
  ...
  const fetchAuditLogs = async (page: number = 1) => {
    ...
    try {
      const response = await api.get<{ logs: AuditLogRecord[]; totalPages: number }>(
  ...
  const [walletDate, setWalletDate] = useState<string>(new Date().toISOString().split('T')[0]);
  ...
  const fetchBillConfig = async () => {
    try {
      const response = await api.get<MonthlyBillConfig>(`/monthly-bill/${monthId}`);
  ...
  const handleDepositChange = async (day: number, userId: string, value: string) => {
  ...
  const handleMealChange = async (day: number, userId: string, change: number) => {
    if (!trackerData) return;
  ...
  const handleBazarChange = async (day: number, userId: string, value: string) => {
    if (!trackerData) return;
    const numericVal = parseFloat(value) || 0;
    ...
    return (
      <div className="min-h-screen w-screen bg-[#FAF6F0] flex flex-col items-center justify-center text-[#4A3728]">
        <div className="text-center space-y-3">
          ...
          <p className="text-xs font-semibold tracking-wider uppercase text-[#8C7662]">Loading Cozy Ledger...</p>
        </div>
      </div>
    );
  }
  ...
  if (!token || !currentUser) {
    return <Auth onAuthSuccess={(t, u) => { setToken(t); setCurrentUser(u); }} />;
  }
      ...
      <aside className="w-64 bg-slate-900 border-r border-slate-800 flex flex-col justify-between shrink-0">
        <div>
          ...
          <div className="h-16 flex items-center px-6 border-b border-slate-800 gap-3">
            ...
            <div>
              <h1 className="font-bold text-lg leading-none tracking-wide text-white font-serif">LifeOS</h1>
              <span className="text-[10px] text-indigo-400 font-extrabold tracking-wider uppercase">Welcome Home</span>
            </div>
          </div>
        ...
        <div className="p-4 border-t border-slate-800 space-y-4">
          ...
          <div className="p-3 rounded-2xl bg-indigo-950/20 border border-indigo-900/35 shadow-inner">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest leading-none">Logged In As</p>
          ...
          {currentUser?.role === 'admin' && (
            <form onSubmit={handleInviteRoommate} className="space-y-2 p-3 bg-slate-900/30 border border-slate-800 rounded-2xl">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Invite Roommate</p>
          ...
          {homeData?.members && homeData.members.length > 1 && (
            <div className="p-3 bg-slate-900/30 border border-slate-800 rounded-2xl space-y-2 mt-2">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Roommate Bill Permissions</p>
              <div className="space-y-1.5 max-h-[140px] overflow-y-auto pr-1">
                {homeData.members.map((member: any) => {
                  if (member._id === homeData.admin) return null; // Hide owner from toggle list
                  const hasControl = (homeData.utilityControlMembers || []).includes(member._id);
                  const canToggle = homeData.admin === currentUser?._id;
                  return (
                    <div key={member._id} className="flex justify-between items-center gap-2 text-[11px] py-1.5 border-b border-slate-850 last:border-b-0">
                      <span className="text-slate-300 font-medium truncate font-sans">@{member.nickname}</span>
                      <div className="flex items-center gap-2">
                        <span className={`text-[9px] font-bold tracking-wider uppercase transition-colors duration-200 ${hasControl ? 'text-indigo-400' : 'text-slate-500'}`}>
                        ...
                        <button
                          ...
                          disabled={!canToggle}
                          onClick={() => handleTogglePermission(member._id, hasControl)}
                        ...
                        >
                          <span
                            className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white transition duration-200 ease-in-out shadow-sm ${
                              hasControl ? 'translate-x-[18px]' : 'translate-x-[2px]'
                            }`}
                          />
                        </button>
                      </div>
            ...
            <div className="flex items-center gap-2 text-slate-300">
              ...
              >
                {availableMonths.map((m) => {
                  const parts = m.split('-');
                  const displayName = parts.length === 2 ? `${parts[0]} ${parts[1]}` : m;
                  return (
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-4 z-50 space-y-3 text-left">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {notifications.length === 0 ? (
                      <p className="text-[10px] text-slate-500 italic py-2 text-center">No notifications yet.</p>
                    ) : (
                      notifications.map(n => (
                        <div
                          key={n.id}
        ...
        <div className="p-8 space-y-8 flex-1">
            ...
            <div className="flex flex-col items-center justify-center min-h-[50vh] animate-fade-in text-center p-8 bg-slate-900/20 border border-slate-800/40 rounded-3xl space-y-6 max-w-2xl mx-auto my-12 backdrop-blur-sm shadow-2xl animate-fade-in">
              <div className="relative">
                ...
                <div className="relative flex items-center justify-center w-16 h-16 rounded-full bg-slate-900 border border-slate-700 text-indigo-400">
                  <Laptop size={32} className="animate-pulse" />
                </div>
              </div>
              
              <div className="space-y-2">
                <h2 className="text-2xl font-bold tracking-tight text-white font-serif">Device Desk</h2>
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black tracking-widest bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 uppercase font-sans">
                  Coming Soon
                </div>
              </div>
              
              <p className="text-slate-400 text-xs max-w-sm leading-relaxed font-medium">
                We are building an intelligent hardware dashboard to check active computer units, system health, and device logs in your household in real-time.
              </p>
              
              <div className="flex gap-2 text-[10px] text-slate-500 font-mono font-bold">
          ...
          {(activeTab as any) === 'hardware-disabled' && (
            <div className="space-y-8">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                  <h2 className="text-2xl font-bold text-white tracking-tight font-serif">Device Desk</h2>
                  <p className="text-slate-400 text-sm">Check active computer units and system health in the house.</p>
                </div>
              ...
              {telemetryLoading ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4">
                  ...
                  <p className="text-slate-400 text-sm font-semibold">Listening for home devices...</p>
                </div>
              ) : telemetryError ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4 bg-slate-900/40 border border-slate-800 rounded-2xl p-8">
                  ...
                  <h3 className="text-lg font-bold text-white font-serif">Device Desk is Quiet</h3>
                  <p className="text-slate-400 text-sm text-center max-w-md">The telemetry database is offline right now. Enjoy the peaceful silence, or click below to check again.</p>
                  <button 
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="space-y-8 flex flex-col justify-between">
                      ...
                      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                        <div>
                          <h3 className="font-bold text-sm text-white">Device Usage Ledger</h3>
                          <span className="text-[10px] text-slate-500 block">Total active usage hours for each registered device</span>
                        </div>

                        <div className="space-y-3">
                          {summaryData?.deviceUsages && summaryData.deviceUsages.length > 0 ? (
                            summaryData.deviceUsages.map(dev => (
                              <div key={dev.deviceId} className="flex justify-between items-center bg-slate-800/35 p-3 rounded-xl border border-slate-800/50">
                                <div>
                                  <span className="font-semibold text-xs text-white block uppercase tracking-wider">{dev.deviceId}</span>
                                  <span className="text-[10px] text-indigo-400 capitalize">Owner: {dev.ownerName}</span>
                                </div>
                                <div className="text-right">
                                  <span className="text-xs font-bold text-indigo-400 block">{dev.usageHours.toFixed(1)} hrs</span>
                                  <span className="text-[10px] text-slate-500">{dev.usagePercent.toFixed(1)}% share</span>
                                </div>
                              </div>
                            ))
                          ) : (
                            <p className="text-slate-500 text-xs py-4 text-center">No active devices right now — all quiet in the house.</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="flex flex-col items-center justify-center h-96 gap-4 bg-slate-900/40 border border-slate-800 rounded-2xl p-8">
                  <span className="text-4xl">🔌</span>
                  <h3 className="text-lg font-bold text-white font-serif">Device Desk is Offline</h3>
                  <p className="text-slate-400 text-sm text-center max-w-sm">No telemetry records exist for this device yet. Start the python telemetry agent to share health snapshots.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'tracker' && (
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                <div>
                  <h2 className="text-2xl font-bold text-white tracking-tight font-serif">Kitchen & Meals</h2>
                  <p className="text-slate-400 text-sm">Log daily roommate meal counts and shared grocery expenses.</p>
                </div>

                <div className="flex items-center gap-4 bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5">
                  <span className="text-xs text-slate-400 font-medium uppercase tracking-wider">Days in Month:</span>
                  <input
                    type="number"
                    min={1}
                    max={31}
                    value={trackerData?.daysInMonth || 30}
                    onChange={(e) => handleDaysConfigChange(parseInt(e.target.value) || 30)}
              ...
              <div className="flex gap-4 border-b border-slate-800 pb-px">
                <button
                  ...
                  className={`pb-3 font-semibold text-sm transition-all relative cursor-pointer ${
                    trackerSubTab === 'meals' ? 'text-indigo-400 border-b-2 border-indigo-500 font-bold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Meals Eaten
                </button>
                <button
                  ...
                  className={`pb-3 font-semibold text-sm transition-all relative cursor-pointer ${
                    trackerSubTab === 'bazar' ? 'text-amber-500 border-b-2 border-amber-550 font-bold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Grocery Bills
                </button>
                <button
                  ...
                  className={`pb-3 font-semibold text-sm transition-all relative cursor-pointer ${
                    trackerSubTab === 'deposits' ? 'text-emerald-450 border-b-2 border-emerald-500 font-bold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Meal Deposits
                </button>
                <button
                  ...
                  className={`pb-3 font-semibold text-sm transition-all relative cursor-pointer ${
                    trackerSubTab === 'wallet' ? 'text-indigo-400 border-b-2 border-indigo-500 font-bold' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  Grocery Wallet
                </button>
              ...
              {trackerLoading ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4">
                  <Loader2 className="h-10 w-10 text-indigo-600 animate-spin" />
                  <p className="text-slate-400 text-sm font-medium">Opening the kitchen cabinets...</p>
                </div>
              ) : trackerError ? (
                <div className="flex flex-col items-center justify-center h-96 gap-4 bg-slate-900/40 border border-slate-800 rounded-2xl p-8">
                  <span className="text-4xl">🍳</span>
                  <h3 className="text-lg font-bold text-white font-serif">Kitchen Snag</h3>
                  <p className="text-slate-400 text-sm text-center max-w-md">We couldn't open the kitchen ledger. Let's try checking the cupboards again.</p>
                </div>
              ) : trackerData ? (
                  ...
                  {trackerSubTab === 'meals' ? (
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h3 className="font-bold text-base text-white">Daily Meals Consumed</h3>
                          <p className="text-xs text-slate-400">Click (+) or (-) to update daily meals.</p>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Total Combined Meals</span>
                          <span className="text-xl font-black text-indigo-400">
                            {trackerData.meals.reduce((acc, item) => acc + Object.values(item.meals).reduce((s, c) => s + c, 0), 0).toFixed(1)}
                          </span>
                        </div>
                      </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px] sticky-header">
                          <thead className="bg-slate-950/95 sticky top-0 z-10">
                            <tr className="border-b border-slate-800 text-slate-400 text-xs font-semibold uppercase tracking-wider">
                              <th className="py-3 px-4 w-24 text-center">Day</th>
                              {trackerData.users.map(u => (
                                <th key={u._id} className="py-3 px-4 text-center">{u.name}</th>
                              ))}
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-800/80">
                            {trackerData.meals.map(item => (
                              <tr key={item.day} className="hover:bg-slate-800/20 text-slate-200 text-xs border-b border-slate-800/40">
                                <td className="py-2.5 px-4 font-bold text-slate-400 text-center bg-slate-950/20">Day {item.day}</td>
                                {trackerData.users.map(u => {
                                  const count = item.meals[u._id] || 0;
                                  return (
                                    <td key={u._id} className="py-2.5 px-4 text-center">
                                      <div className="flex items-center justify-center gap-2">
                                        <button
                                          onClick={() => handleMealChange(item.day, u._id, -0.5)}
                                        ...
                                        <span className={`w-8 font-semibold text-center ${count > 0 ? 'text-indigo-400 font-bold' : 'text-slate-500'}`}>
                                          {count}
                                        </span>
                                        <button
                                          onClick={() => handleMealChange(item.day, u._id, 0.5)}
                            ...
                            <tr className="bg-slate-950/60 border-t border-slate-800 font-bold text-slate-200 text-xs">
                              <td className="py-3 px-4 text-center uppercase tracking-wider">Total</td>
                              {trackerData.users.map(u => {
                                const sum = trackerData.meals.reduce((acc, item) => acc + (item.meals[u._id] || 0), 0);
                                return (
                                  <td key={u._id} className="py-3 px-4 text-center text-indigo-400 font-extrabold text-sm">
                                    {sum.toFixed(1)}
                                  </td>
                                );
                              })}
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h3 className="font-bold text-base text-white">Daily Bazar Expenses</h3>
                          <p className="text-xs text-slate-400">Enter amounts spent on bazar for each roommate. Updates auto-save on blur.</p>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Total Combined Bazar Cost</span>
                          <span className="text-xl font-black text-amber-400">
                            ৳{trackerData.bazar.reduce((acc, item) => acc + Object.values(item.costs).reduce((s, c) => s + c, 0), 0).toFixed(2)}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="overflow-x-auto rounded-xl border border-slate-800 max-h-[500px]">
                        <table className="w-full text-left border-collapse min-w-[600px] sticky-header">
                          <thead className="bg-slate-950/95 sticky top-0 z-10">
                            <tr className="border-b border-slate-800 text-slate-400 text-xs font-semibold uppercase tracking-wider">
                              <th className="py-3 px-4 w-24 text-center">Day</th>
                              <th className="py-3 px-4 text-center w-48">Assigned Roommate</th>
                              {trackerData.users.map(u => (
                                <th key={u._id} className="py-3 px-4 text-center">{u.name}</th>
                              ))}
                            </tr>
                          ...
                          <tbody className="divide-y divide-slate-800/80">
                            {trackerData.bazar.map(item => (
                              <tr key={item.day} className="hover:bg-slate-800/20 text-slate-200 text-xs border-b border-slate-800/40">
                                <td className="py-2 px-4 font-bold text-slate-400 text-center bg-slate-950/20">Day {item.day}</td>
                                <td className="py-2 px-4 text-center">
                                  <select
                                    value={item.assignedUser || ''}
                                    onChange={(e) => handleBazarAssignmentChange(item.day, e.target.value)}
                                ...
                                {trackerData.users.map(u => {
                                  ...
                                  const isCommentOpen = activeCommentCell?.type === 'bazar' && activeCommentCell?.day === item.day && activeCommentCell?.userId === u._id;
                                  const hasNote = !!item.notes?.[u._id];
                                  return (
                                    <td key={u._id} className="py-2 px-4 text-center relative">
                                      <div className="flex items-center justify-center gap-1">
                                        <div className="flex items-center justify-center">
                                          <span className="text-slate-500 mr-0.5 text-[10px] font-bold">৳</span>
                                          <input
                                            id={`bazar-input-${item.day}-${u._id}`}
                                            type="number"
                                            defaultValue={costVal === 0 ? '' : costVal}
                                            placeholder="0"
                                            onBlur={(e) => handleBazarChange(item.day, u._id, e.target.value)}
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            if (isCommentOpen) {
                                              ...
                                              setActiveCommentCell({ type: 'bazar', day: item.day, userId: u._id });
                                              setActiveCommentText(item.notes?.[u._id] || '');
                                            }
                                          }}
                                          className={`p-1 rounded transition-all cursor-pointer ${
                                            hasNote ? 'text-amber-500 bg-amber-500/10' : 'text-slate-500 hover:text-slate-205 hover:bg-slate-800'
                                          }`}
                                          title={item.notes?.[u._id] || 'Add note'}
                                        >
                                      ...
                                      {isCommentOpen && (
                                        <div className="absolute left-1/2 -translate-x-1/2 mt-2 bg-slate-900 border border-slate-800 rounded-xl p-2.5 shadow-xl z-20 w-48 text-left space-y-2">
                                          <label className="text-[9px] uppercase font-bold text-slate-400 block">Bazar Note</label>
                                          <textarea
                                          ...
                                          <div className="flex justify-end gap-1.5">
                                            ...
                                            <button
                                              ...
                                              onClick={async () => {
                                                try {
                                                  const inputEl = document.getElementById(`bazar-input-${item.day}-${u._id}`) as HTMLInputElement;
                                                  const currentAmount = inputEl ? (parseFloat(inputEl.value) || 0) : costVal;
                                                  if (currentAmount <= 0) {
                                                    showAlert('Amount Required', 'Please enter an expense amount before saving a note.');
                                                    return;
                                                  }
                                                  await api.post('/tracker/bazar/update', {
                                                    monthId,
                                                  ...
                                                  fetchTracker();
                                                  setActiveCommentCell(null);
                                                  showAlert('Success', 'Bazar cost and note saved successfully!');
                                                } catch (err) {
                                                  console.error('Error saving bazar note:', err);
                                                }
                                              }}
                            ...
                            <tr className="bg-slate-950/60 border-t border-slate-800 font-bold text-slate-200 text-xs">
                              <td className="py-3 px-4 text-center uppercase tracking-wider">Total</td>
                              <td className="py-3 px-4 text-center text-slate-500 font-medium italic">--</td>
                              {trackerData.users.map(u => {
                                const sum = trackerData.bazar.reduce((acc, item) => acc + (item.costs[u._id] || 0), 0);
                                return (
                                  <td key={u._id} className="py-3 px-4 text-center text-amber-400 font-extrabold text-sm">
                                    ৳{sum.toFixed(2)}
                                  </td>
                                );
                              })}
                    ...
                    <div>
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h3 className="font-bold text-base text-white">Daily Meal Deposits (Given for Meal)</h3>
                          <p className="text-xs text-slate-400">Enter deposits made by each roommate for meals. Updates auto-save on blur.</p>
                        </div>
                        <div className="text-right">
                          <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Total Combined Deposits</span>
                          <span className="text-xl font-black text-emerald-400">
                            ৳{trackerData.deposits.reduce((acc, item) => acc + Object.values(item.amounts).reduce((s, c) => s + c, 0), 0).toFixed(2)}
                          </span>
                        </div>
                      </div>
                        ...
                        <table className="w-full text-left border-collapse min-w-[600px] sticky-header">
                          ...
                          <tbody className="divide-y divide-slate-800/80">
                            {trackerData.deposits.map(item => (
                              <tr key={item.day} className="hover:bg-slate-800/20 text-slate-200 text-xs border-b border-slate-800/40">
                                ...
                                {trackerData.users.map(u => {
                                  ...
                                  const isCommentOpen = activeCommentCell?.type === 'deposit' && activeCommentCell?.day === item.day && activeCommentCell?.userId === u._id;
                                  const hasNote = !!item.notes?.[u._id];
                                  return (
                                    <td key={u._id} className="py-2 px-4 text-center relative">
                                      <div className="flex items-center justify-center gap-1">
                                        <div className="flex items-center justify-center">
                                          ...
                                          <input
                                            ...
                                            type="number"
                                            defaultValue={depVal === 0 ? '' : depVal}
                                            placeholder="0"
                                            onBlur={(e) => handleDepositChange(item.day, u._id, e.target.value)}
                                        ...
                                        <button
                                          ...
                                          onClick={() => {
                                            if (isCommentOpen) {
                                              ...
                                              setActiveCommentCell({ type: 'deposit', day: item.day, userId: u._id });
                                              setActiveCommentText(item.notes?.[u._id] || '');
                                            }
                                          }}
                                          className={`p-1 rounded transition-all cursor-pointer ${
                                            hasNote ? 'text-emerald-400 bg-emerald-400/10' : 'text-slate-500 hover:text-slate-205 hover:bg-slate-800'
                                          }`}
                                          title={item.notes?.[u._id] || 'Add note'}
                                        >
                                          <MessageSquare size={12} />
                                      ...
                                      {isCommentOpen && (
                                        <div className="absolute left-1/2 -translate-x-1/2 mt-2 bg-slate-900 border border-slate-800 rounded-xl p-2.5 shadow-xl z-20 w-48 text-left space-y-2">
                                          <label className="text-[9px] uppercase font-bold text-slate-400 block">Deposit Note</label>
                                          <textarea
                                            value={activeCommentText}
                                            onChange={(e) => setActiveCommentText(e.target.value)}
                                          ...
                                          <div className="flex justify-end gap-1.5">
                                            ...
                                            <button
                                              ...
                                              onClick={async () => {
                                                try {
                                                  const inputEl = document.getElementById(`deposit-input-${item.day}-${u._id}`) as HTMLInputElement;
                                                  const currentAmount = inputEl ? (parseFloat(inputEl.value) || 0) : depVal;
                                                  if (currentAmount <= 0) {
                                                    showAlert('Amount Required', 'Please enter a deposit amount before saving a note.');
                                                    return;
                                                  }
                                                  await api.post('/tracker/deposits/update', {
                                                    monthId,
                                                  ...
                                                  fetchTracker();
                                                  setActiveCommentCell(null);
                                                  showAlert('Success', 'Deposit and note saved successfully!');
                                                } catch (err) {
                                                  console.error('Error saving deposit note:', err);
                                                }
                                              }}
                            ...
                            <tr className="bg-slate-950/60 border-t border-slate-800 font-bold text-slate-200 text-xs">
                              <td className="py-3 px-4 text-center uppercase tracking-wider">Total</td>
                              {trackerData.users.map(u => {
                                const sum = trackerData.deposits.reduce((acc, item) => acc + (item.amounts[u._id] || 0), 0);
                                return (
                                  <td key={u._id} className="py-3 px-4 text-center text-emerald-400 font-extrabold text-sm">
                                    ৳{sum.toFixed(2)}
                                  </td>
                                );
                              })}
                    ...
                    <div className="space-y-8">
                      ...
                      {walletData && (() => {
                        const totalHouseDeposits = walletData.userSummaries.reduce((sum, u) => sum + (u.deposits || 0), 0);
                        const totalHouseSpent = walletData.userSummaries.reduce((sum, u) => sum + u.spent, 0);
                        const totalHouseRemaining = totalHouseDeposits - totalHouseSpent;
                        return (
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden shadow-md">
                              <span className="text-xs font-semibold text-slate-400 uppercase">Total House Deposits</span>
                              <h3 className="text-2xl font-bold mt-2 text-emerald-400 font-serif">৳{totalHouseDeposits.toFixed(2)}</h3>
                              <p className="text-[10px] text-slate-500 mt-1">Total cash collected from everyone</p>
                            </div>
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden shadow-md">
                              <span className="text-xs font-semibold text-slate-400 uppercase">Total Spent on Bazar</span>
                              <h3 className="text-2xl font-bold mt-2 text-amber-500 font-serif">৳{totalHouseSpent.toFixed(2)}</h3>
                              <p className="text-[10px] text-slate-500 mt-1">Total cost of grocery items purchased</p>
                            </div>
                            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden shadow-md">
                              <span className="text-xs font-semibold text-slate-400 uppercase">Total Remaining Cash</span>
                              <h3 className="text-2xl font-bold mt-2 text-indigo-400 font-serif">৳{totalHouseRemaining.toFixed(2)}</h3>
                              <p className="text-[10px] text-slate-500 mt-1">Cash in hand available for upcoming trips</p>
                            </div>
                          </div>
                        );
                      })()}
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="bg-slate-950/40 p-6 border border-slate-800 rounded-2xl space-y-4">
                          ...
                          <form onSubmit={handleAddTransfer} className="space-y-4">
                            ...
                            <div>
                              <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Given To (Taker)</label>
                              <select
                                value={walletTo}
                            ...
                            <div>
                              ...
                              <div className="relative">
                                <span className="absolute left-3 top-1.5 text-xs text-slate-500 font-bold">৳</span>
                                <input
                                  type="number"
                                  placeholder="0.00"
                                  value={walletAmount}
                            ...
                            <div>
                              <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Note / Context</label>
                              <input
                                type="text"
                            ...
                            <div>
                              <label className="block text-[10px] uppercase font-bold text-slate-400 mb-1">Transfer Date</label>
                              <input
                                type="date"
                                value={walletDate}
                        ...
                        <div className="lg:col-span-2 bg-slate-950/20 p-6 border border-slate-800 rounded-2xl space-y-4">
                          <div>
                            <h3 className="font-bold text-base text-white">Wallet Summary</h3>
                            <p className="text-xs text-slate-400">Current cash-in-hand adjustments computed from transfers & spending.</p>
                          </div>
                            ...
                            <table className="w-full text-left border-collapse text-xs">
                              <thead>
                                <tr className="bg-slate-900 border-b border-slate-800 text-slate-400 font-semibold uppercase tracking-wider">
                                  ...
                                  <th className="py-2.5 px-3 text-right text-emerald-400">Meal Deposits</th>
                                  <th className="py-2.5 px-3 text-right text-teal-400">Received (Taker)</th>
                                  <th className="py-2.5 px-3 text-right text-indigo-400">Given (Giver)</th>
                                  <th className="py-2.5 px-3 text-right text-amber-400">Bazar Spent</th>
                              ...
                              <tbody className="divide-y divide-slate-800/80">
                                {walletData?.userSummaries.map((summary) => {
                                  ...
                                  const balance = (summary.deposits || 0) + summary.received - summary.given - summary.spent;
                                  return (
                                    <tr key={summary.userId} className="hover:bg-slate-850/40 text-slate-200">
                                      <td className="py-3.5 px-3 font-semibold text-white">{summary.name}</td>
                                      <td className="py-3.5 px-3 text-right font-medium text-emerald-400">৳{(summary.deposits || 0).toFixed(2)}</td>
                                      <td className="py-3.5 px-3 text-right font-medium text-teal-400">৳{summary.received.toFixed(2)}</td>
                                      <td className="py-3.5 px-3 text-right font-medium text-indigo-400">৳{summary.given.toFixed(2)}</td>
                                      <td className="py-3.5 px-3 text-right font-medium text-amber-400">৳{summary.spent.toFixed(2)}</td>
                                      <td className={`py-3.5 px-3 text-right font-bold ${balance >= 0 ? 'text-emerald-400' : 'text-rose-450'}`}>
                                        {balance >= 0 ? `৳${balance.toFixed(2)} (Remaining)` : `৳${Math.abs(balance).toFixed(2)} (Owes)`}
                                      </td>
                                    </tr>
                                  );
                                })}
                      ...
                      <div className="bg-slate-950/20 p-6 border border-slate-800 rounded-2xl space-y-4">
                        <h3 className="font-bold text-base text-white">Transfer Transaction History</h3>
                        <div className="overflow-x-auto rounded-xl border border-slate-800">
                          <table className="w-full text-left border-collapse text-xs">
                            <thead>
                              <tr className="bg-slate-900 border-b border-slate-800 text-slate-400 font-semibold uppercase tracking-wider">
                            ...
                            <tbody className="divide-y divide-slate-800/80">
                              {walletData && walletData.transfers && walletData.transfers.length > 0 ? (
                                walletData.transfers.map((tx) => {
                                  const giverName = trackerData.users.find(u => u._id === tx.from)?.name || 'Unknown';
                                  const takerName = trackerData.users.find(u => u._id === tx.to)?.name || 'Unknown';
                                  return (
                                    <tr key={tx._id} className="hover:bg-slate-850/40 text-slate-200">
                                      <td className="py-2.5 px-4 text-slate-400">{new Date(tx.date).toLocaleDateString()}</td>
                                      <td className="py-2.5 px-3 font-semibold text-white">{giverName}</td>
                                      <td className="py-2.5 px-3 font-semibold text-white">{takerName}</td>
                                      <td className="py-2.5 px-3 text-right font-bold text-amber-400">৳{tx.amount.toFixed(2)}</td>
                                      <td className="py-2.5 px-4 text-slate-400 italic">{tx.note || '--'}</td>
                                ...
                                <tr>
                                  <td colSpan={6} className="py-8 text-center text-slate-500 italic font-medium">🌱 No transfers recorded yet — enjoy the quiet!</td>
                                </tr>
                              )}
                            </tbody>
                          </table>
          ...
          {activeTab === 'dashboard' && (
            loading ? (
              <div className="flex flex-col items-center justify-center h-96 gap-4">
                <Loader2 className="h-10 w-10 text-indigo-600 animate-spin" />
                <p className="text-slate-400 text-sm font-semibold">Gathering our house records...</p>
              </div>
            ) : error ? (
              <div className="flex flex-col items-center justify-center h-96 gap-4 bg-slate-900/40 border border-slate-800 rounded-2xl p-8">
                <span className="text-4xl">🏡</span>
                <h3 className="text-lg font-bold text-white font-serif">A Cozy Snag</h3>
                <p className="text-slate-400 text-sm text-center max-w-md">We couldn't connect to our home database. Let's try opening the doors again.</p>
                <button 
              ...
              <>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <span className="text-xs font-semibold text-slate-400 uppercase">Shared Meals Cooked</span>
                      ...
                      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
                        ...
                        <span className="text-xs font-semibold text-slate-400 uppercase">Today's Meal Rate</span>
                ...
                <div className="grid grid-cols-1 gap-8">
                      ...
                      <table className="w-full text-left border-collapse min-w-[900px]">
                        <thead>
                          <tr className="bg-slate-900/80 border-b border-slate-800 text-slate-400 text-xs font-semibold uppercase tracking-wider">
                            ...
                            <th className="py-4 px-3 text-right text-orange-400">Spent on Bazar</th>
                            <th className="py-4 px-3 text-right text-amber-400">Food Due</th>
                            <th className="py-4 px-3 text-right text-cyan-400">Utility Due</th>
                            <th className="py-4 px-3 text-right text-indigo-400">Rent Due</th>
                            <th className="py-4 px-3 text-right">Final Due</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/80">
                            ...
                            return (
                              <tr key={user.userId} className="hover:bg-slate-800/25 text-slate-200 text-sm transition-all border-b border-slate-800/40">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <div className="flex flex-col gap-0.5">
                                        ...
                                        {user.note && (
                                          <span className="text-[10px] text-amber-500 italic bg-amber-500/5 px-1.5 py-0.5 rounded border border-amber-500/10 mt-1 max-w-[200px] truncate" title={user.note}>
                                            📝 {user.note}
                                          </span>
                                        )}
                                      </div>
                                ...
                                <td className="py-5 px-3 text-right font-medium text-slate-300">
                                  <div>৳{user.mealCostPortion.toFixed(2)}</div>
                                  {user.prevMealDue !== 0 && (
                                    <span className="text-[10px] text-slate-500 block">Prev: {user.prevMealDue >= 0 ? '+' : ''}৳{user.prevMealDue.toFixed(0)}</span>
                                  )}
                                </td>
                                ...
                                <td className="py-5 px-3 text-right font-semibold text-orange-400">
                                  ...
                                  {user.walletGiven !== 0 || user.walletReceived !== 0 ? (
                                    <span className="text-[10px] text-slate-500 block leading-tight">
                                      Given: ৳{user.walletGiven.toFixed(0)} | Recv: ৳{user.walletReceived.toFixed(0)}<br/>
                                      Net: {user.walletGiven - user.walletReceived >= 0 ? '+' : ''}৳{(user.netBazarPaid || 0).toFixed(0)}
                                    </span>
                                  ) : null}
                                </td>
                                <td className={`py-5 px-3 text-right font-bold ${user.foodDue > 0 ? 'text-rose-450' : 'text-emerald-400'}`}>
                                  {user.foodDue >= 0 ? '' : '+'}৳{Math.abs(user.foodDue).toFixed(2)}
                                </td>
                                <td className="py-5 px-3 text-right font-medium text-slate-300">
                                  <div className={`font-bold ${user.utilityDue > 0 ? 'text-rose-450' : 'text-emerald-400'}`}>
                                    {user.utilityDue >= 0 ? '' : '+'}৳{Math.abs(user.utilityDue).toFixed(2)}
                                  </div>
                                  <span className="text-[10px] text-slate-500 block leading-tight">
                                    Share: ৳{user.utilityShare.toFixed(0)} | Prev: {user.prevUtilityDue >= 0 ? '+' : ''}৳{user.prevUtilityDue.toFixed(0)}<br/>
                                    Paid: -৳{user.utilityPayment.toFixed(0)}
                                  </span>
                                </td>
                                <td className="py-5 px-3 text-right font-medium text-slate-300">
                                  <div className={`font-bold ${user.rentDue > 0 ? 'text-rose-450' : 'text-indigo-300'}`}>
                                    ৳{user.rentDue.toFixed(2)}
                                  </div>
                                  <span className="text-[10px] text-slate-500 block leading-tight">
                                    Rent: ৳{user.rentPortion.toFixed(0)} | Paid: -৳{(user.rentPayment || 0).toFixed(0)}
                                  </span>
                                </td>
                                <td className={`py-5 px-3 text-right font-bold transition-colors ${
                                  isOwed ? 'text-emerald-400' : 'text-rose-450'
                                }`}>
                                  {formattedStanding}
                                </td>
                              </tr>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
                    <div>
                      <h3 className="font-bold text-lg text-white">Shared Utility Category Bills</h3>
                      <p className="text-xs text-slate-400">Breakdown of the shared house utilities for this month.</p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        ...
                        const val = (summaryData.monthlyBill?.utilities as any)?.[util.key] || 0;
                        const noteText = (summaryData.monthlyBill?.utilityNotes as any)?.[util.key] || '';
                        return (
                          <div key={util.key} className="flex flex-col gap-1 p-2.5 rounded-xl bg-slate-950/20 border border-slate-800/60 hover:border-slate-755 transition-all">
                            <div className="flex justify-between items-center text-xs font-bold text-white">
                              <span>{util.label}</span>
                              <span className="text-indigo-400">৳{val.toFixed(2)}</span>
                            </div>
                            {noteText && (
                              <p className="text-[10px] text-amber-500 italic bg-amber-500/5 border border-amber-500/10 px-1.5 py-0.5 rounded leading-normal mt-0.5 hover:bg-amber-500/10 transition-colors" title={noteText}>
                                📌 {noteText}
                              </p>
                            )}
                          </div>
              ...
              <div className="flex flex-col items-center justify-center h-96 text-slate-400 bg-slate-900 border border-slate-800 rounded-2xl p-8 gap-3">
                <span className="text-4xl">🏡</span>
                <h3 className="text-lg font-bold text-white font-serif">A Quiet House</h3>
                <p className="text-sm max-w-sm text-center">No calculations or records exist for this period yet. Enjoy the peaceful calm, or configure your bill details to get started.</p>
              </div>
            )
          )}
          ...
          {activeTab === 'notepad' && (
            <div className="space-y-6 animate-fade-in">
              <div className="flex justify-between items-end">
                <div>
                  <h2 className="text-2xl font-bold text-white font-serif">House Notes & Purchases</h2>
                  <p className="text-slate-400 text-sm">Ad-hoc notes, lists, reminders, and small grocery items (like brooms, cleaning supplies) for the house.</p>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  {noteCategory === 'purchase' ? (
                    <div className="flex items-center animate-fade-in">
                      <span className="text-slate-500 mr-1.5 text-xs font-bold">৳</span>
                      <input
                        type="number"
              ...
              {notesLoading ? (
                <div className="flex flex-col items-center justify-center h-48 gap-3">
                  ...
                  <p className="text-slate-500 text-xs font-medium">Opening the notepad...</p>
                </div>
              ) : notes.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {notes.map(note => {
                    ...
                    const catColors = {
                      ...
                      purchase: 'text-amber-500 bg-amber-500/10 border-amber-500/20',
                      reminder: 'text-rose-450 bg-rose-505/10 border-rose-500/20',
                    ...
                    return (
                      <div
                        ...
                        className={`bg-slate-900 border rounded-2xl p-5 shadow-md flex flex-col justify-between gap-4 transition-all hover:shadow-lg relative overflow-hidden ${
                          note.pinned ? 'border-amber-500 ring-1 ring-amber-500/20' : 'border-slate-800'
                        }`}
                      >
                        {/* Pin ribbon */}
                        {note.pinned && (
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className={`text-[9px] uppercase font-bold px-2 py-0.5 rounded-full border ${catColors[note.category] || catColors.general}`}>
                            ...
                            <div className="flex items-center gap-1.5">
                              <button
                                type="button"
                                onClick={() => handleToggleNotePin(note._id, note.pinned)}
                                className={`p-1 rounded hover:bg-slate-800 transition-colors cursor-pointer ${
                                  note.pinned ? 'text-amber-500' : 'text-slate-500 hover:text-slate-300'
                                }`}
                          ...
                          {isEditing ? (
                            <div className="space-y-3">
                              ...
                              <div className="flex gap-2">
                                <select
                                  ...
                                  onChange={(e) => setEditingNoteCategory(e.target.value as any)}
                                ...
                                {editingNoteCategory === 'purchase' ? (
                                  <input
                                    ...
                                    onChange={(e) => setEditingNoteAmount(e.target.value)}
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => setEditingNoteReminderDate(e.target.value)}
                                    onClick={(e) => e.currentTarget.showPicker()}
                            ...
                            <div className="space-y-2">
                              <p className={`text-xs font-medium leading-relaxed whitespace-pre-wrap ${
                                note.category === 'todo' && note.completed ? 'line-through text-slate-500' : 'text-slate-100'
                              }`}>{note.text}</p>
                              {note.category === 'purchase' && note.amount > 0 && (
                                <p className="text-sm font-black text-amber-500 font-serif">৳{note.amount.toFixed(2)}</p>
                              )}
                              {note.category === 'reminder' && note.reminderDate && (
                        ...
                        {!isEditing && (
                          <div className="flex justify-between items-center border-t border-slate-800/60 pt-3 text-[10px] text-slate-500 font-medium">
                            <span className="truncate">By {note.createdByName || 'Someone'}</span>
                            ...
                            <div className="flex items-center gap-2">
                              {note.category === 'todo' && (
                                <button
                                  type="button"
                                  onClick={() => handleToggleTodoCompleted(note._id, !!note.completed)}
                                  className={`font-bold transition-colors cursor-pointer ${
                                    note.completed ? 'text-emerald-500 hover:text-emerald-400' : 'text-slate-400 hover:text-white'
                                  }`}
                                >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  setEditingNoteReminderDate(note.reminderDate ? new Date(note.reminderDate).toISOString().substring(0, 16) : '');
                                }}
                ...
                <div className="flex flex-col items-center justify-center h-48 bg-slate-900 border border-slate-800 rounded-2xl p-6 gap-2 text-slate-400 italic bg-slate-900">
                  ...
                  <span className="text-[10px]">Add shopping items, home notes, or reminders above!</span>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === 'history' && (
            <div className="space-y-6 animate-fade-in">
              <div>
                <h2 className="text-2xl font-bold text-white font-serif">Change History Log</h2>
                <p className="text-slate-400 text-sm">Full audit trail of all household modifications, transactions, configurations, and notes for {monthId}.</p>
              </div>

              {auditLoading ? (
                <div className="flex flex-col items-center justify-center h-48 gap-3">
                  <Loader2 className="h-8 w-8 text-indigo-650 animate-spin" />
                  <p className="text-slate-500 text-xs font-medium">Opening the history books...</p>
                </div>
              ) : auditLogs.length > 0 ? (
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-md overflow-hidden">
                    <div className="divide-y divide-slate-850">
                      {auditLogs.map((log) => {
                        ...
                        return (
                          <div key={log._id} className="py-3.5 flex flex-col md:flex-row justify-between items-start md:items-center gap-2 first:pt-1 last:pb-1 text-xs">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <p className="text-xs text-slate-100 font-medium">
                                  {log.changes?.[0]?.detail || `${log.userName} triggered ${log.action}`}
                                </p>
                                <span className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block mt-0.5">
                                  Action by: <span className="text-indigo-400 font-bold">{log.userName}</span>
                                </span>
                              </div>
                            </div>
        ...
        <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-md flex items-center justify-center z-50 overflow-y-auto p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-4xl rounded-2xl p-6 shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b border-slate-800 pb-4">
              <div>
                <h3 className="text-xl font-bold text-white font-serif">Configure Rent & Utilities</h3>
                <p className="text-slate-400 text-xs mt-0.5 font-medium">Set up roommate rent shares, shared utilities, and previous adjustments for {monthId}.</p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveBillConfig} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="space-y-4 bg-slate-950/40 p-4 border border-slate-800 rounded-xl">
                  <h4 className="font-bold text-xs text-indigo-400 uppercase tracking-wider">Home Rent Settings</h4>
                  <div className="space-y-3">
                    {summaryData?.userStandings.map(u => {
                      const rentVal = billConfig.rent[u.userId] || 0;
                      return (
                        <div key={u.userId} className="flex justify-between items-center gap-2">
                          <label className="text-xs text-slate-300 font-medium">{u.name}</label>
                          <div className="flex items-center">
                            <span className="text-[10px] text-slate-500 mr-1">৳</span>
                            <input
                              type="number"
                              value={rentVal}
                              onChange={(e) => setBillConfig({
                                ...billConfig,
                                rent: {
                                  ...billConfig.rent,
                ...
                <div className="space-y-4 bg-slate-950/40 p-4 border border-slate-800 rounded-xl">
                  <h4 className="font-bold text-xs text-indigo-400 uppercase tracking-wider font-semibold">Utility Category Bills</h4>
                  <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                    {Object.keys(billConfig.utilities || {}).map(utilKey => {
                      ...
                      const hasPermission = isOwner || (homeData?.utilityControlMembers || []).includes(currentUser?._id);
                      ...
                      return (
                        <div key={utilKey} className="space-y-1 border-b border-slate-800/40 pb-2 last:border-b-0">
                          <div className="flex justify-between items-center gap-2">
                            <div className="flex items-center gap-1 min-w-0">
                              <label className="text-xs text-slate-300 font-medium truncate" title={getCategoryLabel(utilKey)}>
                              ...
                              {hasPermission && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    const updatedUtils = { ...billConfig.utilities };
                                    delete (updatedUtils as any)[utilKey];
                                    const updatedNotes = { ...(billConfig.utilityNotes || {}) };
                                    delete (updatedNotes as any)[utilKey];
                                    setBillConfig({ ...billConfig, utilities: updatedUtils, utilityNotes: updatedNotes });
                                  }}
                              ...
                              <input
                                ...
                                onChange={(e) => setBillConfig({
                                  ...
                                  utilities: {
                                    ...
                                    [utilKey]: parseFloat(e.target.value) || 0
                                  }
                                })}
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="pt-3 border-t border-slate-800/80 mt-3 space-y-2">
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-sans">Add Utility Category</p>
                          <div className="flex gap-1">
                            ...
                            <button
                              type="button"
                              onClick={() => {
                                const inputEl = document.getElementById('new-utility-category') as HTMLInputElement;
                                const newCat = inputEl?.value?.trim();
                                if (!newCat) return;
                                const normalizedKey = newCat.toLowerCase().replace(/[^a-z0-9]/g, '');
                                if (!normalizedKey) return;
                                if ((billConfig.utilities as any)[normalizedKey] !== undefined) {
                ...
                <div className="space-y-4 bg-slate-950/40 p-4 border border-slate-800 rounded-xl">
                  <h4 className="font-bold text-xs text-indigo-400 uppercase tracking-wider">Dues & Payments Ledger</h4>
                  <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
                    {billConfig.adjustments.map((adj, index) => {
                      const user = summaryData?.userStandings.find(u => u.userId === adj.user);
                      if (!user) return null;
                      return (
                        <div key={adj.user} className="border-b border-slate-800/60 pb-3 space-y-2 last:border-b-0">
                          <p className="text-xs font-bold text-white">{user.name}</p>
                          
                          <div className="flex justify-between items-center gap-2">
                            <span className="text-[10px] text-slate-400">Prev Utility Due</span>
                            <input
                              type="number"
                              value={adj.prevUtilityDue}
                              onChange={(e) => {
                                const newAdj = [...billConfig.adjustments];
                                newAdj[index] = { ...adj, prevUtilityDue: parseFloat(e.target.value) || 0 };
                                setBillConfig({ ...billConfig, adjustments: newAdj });
                              }}
                          ...
                          <div className="flex justify-between items-center gap-2">
                            <span className="text-[10px] text-slate-400">Prev Meal Due</span>
                            <input
                              type="number"
                              value={adj.prevMealDue}
                              onChange={(e) => {
                                const newAdj = [...billConfig.adjustments];
                                newAdj[index] = { ...adj, prevMealDue: parseFloat(e.target.value) || 0 };
                                setBillConfig({ ...billConfig, adjustments: newAdj });
                              }}
                              className="bg-slate-900 border border-slate-800 rounded px-2 py-0.5 w-20 text-right focus:outline-none text-[10px] text-white"
                            />
                          ...
                          <div className="flex justify-between items-center gap-2">
                            <span className="text-[10px] text-slate-400">Utility Paid (New Given)</span>
                            <input
                              type="number"
                              value={adj.utilityPayment}
                              onChange={(e) => {
                                const newAdj = [...billConfig.adjustments];
                                newAdj[index] = { ...adj, utilityPayment: parseFloat(e.target.value) || 0 };
                                setBillConfig({ ...billConfig, adjustments: newAdj });
                              }}
                              className="bg-slate-900 border border-slate-800 rounded px-2 py-0.5 w-20 text-right focus:outline-none text-[10px] text-white"
                            />
                          ...
                          <div className="flex justify-between items-center gap-2">
                            <span className="text-[10px] text-slate-400">Rent Paid (New Given)</span>
                            <input
                              type="number"
                              value={adj.rentPayment || 0}
                              onChange={(e) => {
                                const newAdj = [...billConfig.adjustments];
                                newAdj[index] = { ...adj, rentPayment: parseFloat(e.target.value) || 0 };
                                setBillConfig({ ...billConfig, adjustments: newAdj });
                              }}
                              className="bg-slate-900 border border-slate-800 rounded px-2 py-0.5 w-20 text-right focus:outline-none text-[10px] text-white"
                            />
      ...
      {isNewMonthModalOpen && (
        <div className="fixed inset-0 bg-slate-950/75 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl p-6 shadow-2xl space-y-6">
            <div>
              <h3 className="text-xl font-bold text-white font-serif">Create New Month</h3>
              <p className="text-slate-400 text-xs mt-0.5">Initialize the next calendar month. Dues and standing calculations will automatically carry forward.</p>
            </div>

            <div className="space-y-4">
              <div className="p-3 bg-slate-950/40 border border-slate-800 rounded-xl space-y-2">
                <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider block">Base Month (Carry from)</span>
                <select
              ...
              <div className="p-3 bg-indigo-950/20 border border-indigo-900/30 rounded-xl text-xs text-indigo-300 leading-relaxed">
                ...
                The system will automatically calculate the surplus/deficit for everyone and apply them as **Prev Utility Due** and **Prev Meal Due** in the new month config.
              </div>
            </div>
